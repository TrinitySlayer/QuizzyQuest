...
func _ready():
	set_health($CikguAgamaContainer/ProgressBar, enemy.health, enemy.health)
	set_health($PlayerPanel/PlayerData/ProgressBar, State.current_health, State.max_health)
	$CikguAgamaContainer/CikguAgama.texture = enemy.texture
	...
	display_text(" Alamak!! Ada penjaga cuba menyerang. ")
	await textbox_closed
	$ActionPanel.show()
	
	display_text("Penjaga : Hai, manusia jawablah pertanyaanku.
...
func _on_jawapan_2_pressed():
	...
	set_health($CikguAgamaContainer/ProgressBar, current_enemy_health, enemy.health)
	...
	if current_enemy_health == 0:
		...
		await get_tree().create_timer(.25).timeout
		get_tree().change_scene_to_file("res://scenes/moste_win.tscn") #Jangan tukar
