...
func _ready():
	set_health($CikguSej2Container/ProgressBar, enemy.health, enemy.health)
	set_health($PlayerPanel/PlayerData/ProgressBar, State.current_health, State.max_health)
	$CikguSej2Container/CikguSej2.texture = enemy.texture
	...
	display_text(" Penjaga : Hei, manusia. Jawab soalan aku! 
	 Namakan ibu negeri bagi negeri Pahang? ")
	await textbox_closed
	$ActionPanel.show()
...
func _on_jawapan_3_pressed():
	...
	set_health($CikguSej2Container/ProgressBar, current_enemy_health, enemy.health)
	...
	if current_enemy_health == 0:
		...
		await get_tree().create_timer(.25).timeout
		get_tree().change_scene_to_file("res://scenes/final_one_win.tscn") #Jangan tukar
