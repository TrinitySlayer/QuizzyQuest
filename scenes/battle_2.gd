...
func _ready():
	set_health($CikguSejContainer/ProgressBar, enemy.health, enemy.health)
	set_health($PlayerPanel/PlayerData/ProgressBar, State.current_health, State.max_health)
	$CikguSejContainer/CikguSej.texture = enemy.texture
	...
	display_text(" Alamak! Ada penjaga yang cuba untuk menyerang. ")
	await textbox_closed
	$ActionPanel.show()
	
	display_text(" Penjaga : Siapakah Bapa Kemerdekaan negara kita? " )
	await textbox_closed
	$ActionPanel.show()
...
func _on_jawapan_1_pressed():
	...
	if current_player_health == 0:
		display_text(" Anda gagal menewaskan penjaga. ")
		await textbox_closed
		$playerDie.play()
		await get_tree().create_timer(.25).timeout
...
func _on_jawapan_2_pressed():
	...
	set_health($CikguSejContainer/ProgressBar, current_enemy_health, enemy.health)
	...
	if current_enemy_health == 0:
		display_text(" Anda berjaya menewaskan penjaga, 
		...
		await get_tree().create_timer(.25).timeout
		get_tree().change_scene_to_file("res://scenes/sej_win.tscn") #Jangan tukar
