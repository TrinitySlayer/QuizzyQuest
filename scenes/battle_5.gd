...
func _ready():
	set_health($CikguSej3Container/ProgressBar, enemy.health, enemy.health)
	set_health($PlayerPanel/PlayerData/ProgressBar, State.current_health, State.max_health)
	$CikguSej3Container/CikguSej3.texture = enemy.texture
...
func _on_jawapan_1_pressed():
	...
	if current_player_health == 0:
		display_text(" Anda tidak berjaya mengalahkan penjaga. " % enemy.name)
		await textbox_closed
...
func _on_jawapan_2_pressed():
	...
	set_health($CikguSej3Container/ProgressBar, current_enemy_health, enemy.health)
	...
	if current_enemy_health == 0:
		display_text(" Anda berjaya mengalahkan penjaga. Sila ke tempat seterusnya. " )
		await textbox_closed
		...
		await get_tree().create_timer(.25).timeout
		get_tree().change_scene_to_file("res://scenes/final_two_win.tscn") #Jangan tukar
