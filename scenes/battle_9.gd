...
func _ready():
	set_health($CikguSej7Container/ProgressBar, enemy.health, enemy.health)
	set_health($PlayerPanel/PlayerData/ProgressBar, State.current_health, State.max_health)
	$CikguSej7Container/CikguSej7.texture = enemy.texture
	...
	display_text(" Penjaga : Hei, manusia. Jawab pertanyaanKu! 
	 Anak bulan & bintang pada bendera Malaysia membawa maksud? ")
	await textbox_closed
	$ActionPanel.show()
...
func _on_jawapan_3_pressed():
	...
	set_health($CikguSej7Container/ProgressBar, current_enemy_health, enemy.health)
	...
	if current_enemy_health == 0:
		...
		await get_tree().create_timer(.25).timeout
		get_tree().change_scene_to_file("res://scenes/final_six_win.tscn") #Jangan tukar
